...
Print File Function 
*/
uint8_t print_file(uint32_t Cluster, uint8_t xdata * array_in)
{
	uint32_t sector, sector_offset = 0, input;
	...
	do{
		Read_Sector((sector + sector_offset), Drive_Value->BytesPerSec, array_in);
		print_memory(array_in, 512);
		sector_offset++;
		if(sector_offset == Drive_Value->SecPerClus)
		{
